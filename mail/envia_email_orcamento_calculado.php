...
// =-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
//$mail->AddAddress('rogerio@mogicomp.com.br');

$sql_admin = "SELECT * FROM cadastro_clientes
			  WHERE cli_email <> '' AND cli_id = $orc_cliente ";
...
if($rows_admin > 0)
{
	...
	{
		$email_cliente = mysql_result($query_admin,$x,'cli_email');
...
// =-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
$mail->Subject  = "".utf8_decode("Orçamento Calculado").""; // Assunto da mensagem
