...
if (isset($_GET['action']) && $_GET['action'] === 'aprovar' && isset($_GET['orc_id'])) {
	...
	$pdo->prepare("UPDATE orcamento_gerenciar SET orc_data_aprovacao = :data WHERE orc_id = :id")
		->execute([':data' => $dataAprovacao, ':id' => $orcId]);
...
if (isset($_GET['action']) && $_GET['action'] === 'reprovar' && isset($_GET['orc_id'])) {
	$orcId = (int) $_GET['orc_id'];
	$dataReprovacao = date('Y-m-d');
	$observacao = $_POST['sto_observacao'] ?? '';

	$pdo->prepare("UPDATE orcamento_gerenciar SET orc_data_aprovacao = :data WHERE orc_id = :id")
		->execute([':data' => $dataReprovacao, ':id' => $orcId]);
...
$fornecedores = [];
$stmtForn = $pdo->query("SELECT for_id, for_nome_razao FROM cadastro_fornecedores ORDER BY for_nome_razao");
if ($stmtForn) {
