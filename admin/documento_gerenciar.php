...
if ($action === "adicionar" || $action === "editar") {
	...
	if ($doc_data_emissao && $doc_periodicidade) {
		...
		switch ($doc_periodicidade) {
			case 6: $date->modify('+6 months'); break;
			case 12: $date->modify('+1 year'); break;
			case 24: $date->modify('+2 years'); break;
			case 36: $date->modify('+3 years'); break;
			case 48: $date->modify('+4 years'); break;
			case 60: $date->modify('+5 years'); break;
		}
		...
		if ($stmt->execute()) {
			...
			foreach ((array)$doc_anexo as $k => $value) {
				if ($doc_anexo[$k] != '') {
					...
					$arquivo = $caminho . md5(mt_rand(1, 10000) . $doc_anexo[$k]) . '.' . $extensao;
					move_uploaded_file($tmp_anexo[$k], $arquivo);
					if ($anexo && file_exists($anexo)) unlink($anexo);
					$stmt3 = $pdo->prepare("UPDATE documento_gerenciar SET doc_anexo = :arquivo WHERE doc_id = :doc_id");
					if (!$stmt3->execute([':arquivo' => $arquivo, ':doc_id' => $doc_id])) $erro = true;
				}
			}
			if (!$erro) {
...
if ($fil_nome) $params[':fil_nome'] = "%$fil_nome%";
if ($fil_doc_tipo) $params[':fil_doc_tipo'] = $fil_doc_tipo;
if ($fil_data_inicio && $fil_data_fim) {
...
if ($pagina == "documento_gerenciar") {
	...
	if ($rows > 0) {
		...
		// Monta query string base para manter filtros na paginação
    ...
    <p>
        <center>
            ...
            <input type='button' id='botao_cancelar'
                onclick=javascript:window.location.href='documento_gerenciar.php?pagina=documento_gerenciar$autenticacao'
                ; value='Cancelar' />
        </center>
        </center>
        </td>
