...
if($action == "adicionar")
{
	$tps_nome = addslashes($_POST['tps_nome']);
	$sql = "INSERT INTO cadastro_tipos_servicos (
	tps_nome
	) 
	VALUES 
	(
...
if($action == 'editar')
{
	$tps_id = $_GET['tps_id'];
	$tps_nome = addslashes($_POST['tps_nome']);
	$sqlEnviaEdit = "UPDATE cadastro_tipos_servicos SET 
					 tps_nome = '$tps_nome'
					 WHERE tps_id = $tps_id ";
...
{
	$tps_id = $_GET['tps_id'];
	$sql = "DELETE FROM cadastro_tipos_servicos WHERE tps_id = '$tps_id'";
	...
	else
	{
		...
		<SCRIPT language='JavaScript'>
			abreMask(
			'<img src=../imagens/x.png> Este tipo de serviço não pode ser excluído pois está relacionado com alguma tabela.<br><br>'+
			'<input value=\' Ok \' type=\'button\' onclick=javascript:window.history.back(); >');
		</SCRIPT>
		";
	}
...
$sql = "SELECT * FROM cadastro_tipos_servicos 
		ORDER BY tps_nome ASC
		LIMIT $primeiro_registro, $num_por_pagina ";
$cnt = "SELECT COUNT(*) FROM cadastro_tipos_servicos ";
...
if($pagina == "cadastro_tipos_servicos")
{
	...
	<div class='centro'>
		...
		<div id='botoes'><input value='Novo Serviço' type='button' onclick=javascript:window.location.href='cadastro_tipos_servicos.php?pagina=adicionar_cadastro_tipos_servicos".$autenticacao."'; /></div>
		";
		if ($rows > 0)
		{
			echo "
			<table align='center' width='100%' border='0' cellspacing='0' cellpadding='10' class='bordatabela'>
				...
				for($x = 0; $x < $rows ; $x++)
				{
					$tps_id = mysql_result($query, $x, 'tps_id');
		...
		<table align='center' cellspacing='0' width='500'>
			<tr>
				<td align='center'>
					...
					<input type='button' id='botao_cancelar' onclick=javascript:window.location.href='cadastro_tipos_servicos.php?pagina=cadastro_tipos_servicos".$autenticacao."'; value='Cancelar'/></center>
					</center>
				</td>
			</tr>
		</table>
...
{
	$tps_id = $_GET['tps_id'];
	$sqledit = "SELECT * FROM cadastro_tipos_servicos
				WHERE tps_id = '$tps_id'";
