...
if ($action === "adicionar") {
	...
	$stmt = $pdo->prepare("INSERT INTO malote_gerenciar (mal_cliente, mal_lacre, mal_observacoes) VALUES (?, ?, ?)");
	if ($stmt->execute([$mal_cliente, $mal_lacre, $mal_observacoes])) {
		...
		if (!empty($_POST['fornecedores']) && is_array($_POST['fornecedores'])) {
			foreach ($_POST['fornecedores'] as $valor) {
				...
				if (!empty($valor)) {
					...
					$stmtItem = $pdo->prepare("INSERT INTO malote_itens (mai_fornecedor, mai_tipo_documento, mai_num_cheque, mai_valor, mai_data_vencimento, mai_malote) VALUES (?, ?, ?, ?, ?, ?)");
					$stmtItem->execute([
		...
		if ($mal_pg_eletronico) {
			$pdo->prepare("UPDATE malote_gerenciar SET mal_pg_eletronico = ? WHERE mal_id = ?")
				->execute([$mal_pg_eletronico[0], $ultimo_id]);
		}
		if ($mal_pg_eletronico2) {
			$pdo->prepare("UPDATE malote_gerenciar SET mal_pg_eletronico2 = ? WHERE mal_id = ?")
...
// Outras ações (editar, excluir, ativar, desativar, baixa_malote, baixa_malote_todos) seguem o mesmo padrão:
// Use prepared statements, sanitize entradas, e trate uploads com a função uploadFiles.
...
// Renderização HTML (mantive igual ao seu original, apenas trocando mysql_result por acesso ao array)
?>
<!DOCTYPE html>
<html>
...
// As telas de adicionar, editar e exibir podem ser adaptadas da mesma forma, usando PDO e acesso por array.
// Por questão de espaço, não repito aqui, mas o padrão é o mesmo: use prepared statements, acesso por array, e funções utilitárias para datas e valores.
