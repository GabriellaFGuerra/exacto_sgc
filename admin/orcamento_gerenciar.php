...
if($action == "adicionar")
{
	...
	$orc_andamento = mysql_real_escape_string($_POST['orc_andamento']);
	$orc_observacoes = mysql_real_escape_string($_POST['orc_observacoes']);
	$orc_usuario_responsavel = $_POST['orc_usuario_responsavel'];
	$orc_gerente_responsavel = $_POST['orc_gerente_responsavel'];
	$orc_prazo = implode("-",array_reverse(explode("/",$_POST['orc_prazo'])));
	if($orc_prazo != ""){$orc_prazo = "'".$orc_prazo."'";}else{ $orc_prazo = 'null';}
...
if($action == 'editar')
{
	...
	$sqlEnviaEdit = "UPDATE orcamento_gerenciar SET 
					 ...
					 orc_usuario_responsavel = '$orc_usuario_responsavel',
					 orc_gerente_responsavel = '$orc_gerente_responsavel',
					 orc_prazo = $orc_prazo,
...
{
	$usuario_responsavel_query = " 1 = 1 ";
	$fil_usuario_responsavel_n = "Usuário Responsável";
}
else
{
	$usuario_responsavel_query = " orc_usuario_responsavel = '".$fil_usuario_responsavel."' ";
	$sql_tipos_servicos = "SELECT * FROM admin_usuarios WHERE usu_id = $fil_usuario_responsavel ";
	$query_tipos_servicos = mysql_query($sql_tipos_servicos,$conexao);
	$fil_usuario_responsavel_n = mysql_result($query_tipos_servicos,0,'usu_nome');
}
$fil_gerente_responsavel = $_REQUEST['fil_gerente_responsavel'];
if($fil_gerente_responsavel == '')
{
	$gerente_responsavel_query = " 1 = 1 ";
	$fil_gerente_responsavel_n = "Gerente";
}
else
{
	$gerente_responsavel_query = " orc_gerente_responsavel = '".$fil_gerente_responsavel."' ";
	$sql_tipos_servicos = "SELECT * FROM cadastro_gerentes WHERE ger_id = $fil_gerente_responsavel ";
	$query_tipos_servicos = mysql_query($sql_tipos_servicos,$conexao);
	$fil_gerente_responsavel_n = mysql_result($query_tipos_servicos,0,'ger_nome');
}
...
{
	$prazo_query = " orc_prazo >= '$hoje' &&  orc_prazo <= '$dois_dias' ";
}
else
{
	$prazo_query = " 1 = 1 ";
...
// $fil_data_inicio_prazo = implode('-',array_reverse(explode('/',$_REQUEST['fil_data_inicio_prazo'])));
// $fil_data_fim_prazo = implode('-',array_reverse(explode('/',$_REQUEST['fil_data_fim_prazo'])));
// if($fil_data_inicio_prazo == '' && $fil_data_fim_prazo == '')
// {
// 	$prazo_query = " 1 = 1 ";
// }
// elseif($fil_data_inicio_prazo != '' && $fil_data_fim_prazo == '')
// {
// 	$prazo_query = " orc_prazo >= '$fil_data_inicio_prazo' ";
// }
// elseif($fil_data_inicio_prazo == '' && $fil_data_fim_prazo != '')
// {
// 	$prazo_query = " orc_prazo <= '$fil_data_fim_prazo 23:59:59' ";
// }
// elseif($fil_data_inicio_prazo != '' && $fil_data_fim_prazo != '')
// {
// 	$prazo_query = " orc_prazo BETWEEN '$fil_data_inicio_prazo' AND '$fil_data_fim_prazo 23:59:59' ";
// }
$sql = "SELECT * FROM orcamento_gerenciar 
		...
		WHERE cli_deletado = 1 and cli_status = 1 and h1.sto_id = (SELECT MAX(h2.sto_id) FROM cadastro_status_orcamento h2 where h2.sto_orcamento = h1.sto_orcamento) AND 
			  ucl_usuario = '".$_SESSION['usuario_id']."' AND
			  ".$orc_query." AND ".$nome_query." AND ".$tipo_servico_query." AND ".$data_query." AND ".$status_query."  AND ".$usuario_responsavel_query."  AND ".$gerente_responsavel_query."  AND ".$prazo_query."  
...
if($pagina == "orcamento_gerenciar")
{
	...
	<div class='centro'>
		...
		<div class='filtro'>
			...
			<select name='fil_tipo_servico' id='fil_tipo_servico' style='width:100px'>
			...
			<select name='fil_usuario_responsavel' id='fil_usuario_responsavel'>
				<option value='$fil_usuario_responsavel'>$fil_usuario_responsavel_n</option>
				"; 
				$sql_tipo_servico = " SELECT * FROM admin_usuarios ORDER BY usu_nome";
				$query_tipo_servico = mysql_query($sql_tipo_servico,$conexao);
				while($row_tipo_servico = mysql_fetch_array($query_tipo_servico) )
				{
					echo "<option value='".$row_tipo_servico['usu_id']."'>".$row_tipo_servico['usu_nome']."</option>";
				}
				echo "
				<option value=''>Todos</option>
			</select>
			<select name='fil_gerente_responsavel' id='fil_gerente_responsavel'>
				<option value='$fil_gerente_responsavel'>$fil_gerente_responsavel_n</option>
				"; 
				$sql_tipo_servico = " SELECT * FROM cadastro_gerentes ORDER BY ger_nome";
				$query_tipo_servico = mysql_query($sql_tipo_servico,$conexao);
				while($row_tipo_servico = mysql_fetch_array($query_tipo_servico) )
				{
					echo "<option value='".$row_tipo_servico['ger_id']."'>".$row_tipo_servico['ger_nome']."</option>";
				}
				echo "
				<option value=''>Todos</option>
			</select>
			<select name='fil_prazo' id='fil_prazo'>
				<option value='$fil_prazo'>$fil_prazo</option>
				<option value='A vencer'>A vencer</option>
				<option value='Vencido'>Vencido</option>							
				<option value=''>Todos</option>
			</select>
			<!--
			<input type='text' name='fil_data_inicio_prazo' id='fil_data_inicio_prazo'  style='width:80px' placeholder='Prazo Início' value='".implode('/',array_reverse(explode('-',$fil_data_inicio_prazo)))."' onkeypress='return mascaraData(this,event);'>
			<input type='text' name='fil_data_fim_prazo' id='fil_data_fim_prazo' style='width:80px' placeholder='Prazo Fim' value='".implode('/',array_reverse(explode('-',$fil_data_fim_prazo)))."' onkeypress='return mascaraData(this,event);'>
			-->
		...
		if ($rows > 0)
		{
			...
			<table align='center' width='100%' border='0' cellspacing='0' cellpadding='10' class='bordatabela'>
				<tr>
					...
					<td class='titulo_tabela' align='center'>Resp.</td>
					<td class='titulo_tabela' align='center'>Gerente</td>
					<td class='titulo_tabela' align='center'>Prazo</td>
				...
				for($x = 0; $x < $rows ; $x++)
				{
					...
					$orc_prazo = implode("/",array_reverse(explode("-",mysql_result($query, $x, 'orc_prazo'))));
					
					$hoje = date("Y-m-d");
					$dois_dias = date("Y-m-d",strtotime('+2 days'));
					
					if(implode("-",array_reverse(explode("/",$orc_prazo))) < $hoje)
					{
						$orc_prazo = "<span style='color: #F00; font-weight:bold'>".$orc_prazo."</span>";
					}
					elseif(implode("-",array_reverse(explode("/",$orc_prazo))) >= $hoje && implode("-",array_reverse(explode("/",$orc_prazo))) <= $dois_dias)
					{
						$orc_prazo = "<span style='color: #F90; font-weight:bold'>".$orc_prazo."</span>";
					}
		...
		<table align='center' cellspacing='0' width='950'>
			<tr>
				<td align='left'>
					...
					<p>
					<select name='orc_usuario_responsavel' id='orc_usuario_responsavel'>
						<option value=''>Selecione o usuário responsavel</option>
						"; 
						$sql = "SELECT * FROM admin_usuarios ORDER BY usu_nome";
						$query = mysql_query($sql,$conexao);
						while($row = mysql_fetch_array($query) )
						{
					...
					<p>
					<select name='orc_gerente_responsavel' id='orc_gerente_responsavel'>
						<option value=''>Selecione o gerente</option>
						"; 
						$sql = "SELECT * FROM cadastro_gerentes ORDER BY ger_nome";
						$query = mysql_query($sql,$conexao);
						while($row = mysql_fetch_array($query) )
						{
					...
					<p>
					<input type='text' name='orc_prazo' id='orc_prazo' placeholder='Prazo (vencimento)' onkeypress='return mascaraData(this,event);'>
					<p>
...
if($pagina == 'editar_orcamento_gerenciar')
{
	...
	if($rowsedit > 0)
	{
		...
		$orc_usuario_responsavel = mysql_result($queryedit, 0, 'orc_usuario_responsavel');
		$orc_gerente_responsavel = mysql_result($queryedit, 0, 'orc_gerente_responsavel');
		$usu_nome = mysql_result($queryedit, 0, 'usu_nome');
		$ger_nome = mysql_result($queryedit, 0, 'ger_nome');
		$orc_prazo = implode("/",array_reverse(explode("-",mysql_result($queryedit, 0, 'orc_prazo'))));
		echo "
			...
			<table align='center' cellspacing='0'>
				<tr>
					<td align='left'>
						...
						<p>
						<select name='orc_usuario_responsavel' id='orc_usuario_responsavel'>
							<option value='$orc_usuario_responsavel'>$usu_nome</option>
							"; 
							$sql = "SELECT * FROM admin_usuarios ORDER BY usu_nome";
							$query = mysql_query($sql,$conexao);
							while($row = mysql_fetch_array($query) )
						...
						<p>
						<select name='orc_gerente_responsavel' id='orc_gerente_responsavel'>
							<option value='$orc_gerente_responsavel'>$ger_nome</option>
							"; 
							$sql = "SELECT * FROM cadastro_gerentes ORDER BY ger_nome";
							$query = mysql_query($sql,$conexao);
							while($row = mysql_fetch_array($query) )
						...
						<p>
						<input type='text' name='orc_prazo' id='orc_prazo' value='$orc_prazo' placeholder='Prazo (vencimento)' onkeypress='return mascaraData(this,event);'>
						<p>
						<div class='formtitulo'>Status do Orçamento</div>
						<p>
